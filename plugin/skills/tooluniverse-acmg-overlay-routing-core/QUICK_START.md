...
3. Present final ACMG/pathogenicity wording only when the controller returns `validator_status: "PASS"`, `semantic_combiner_status: "PASS"`, and `final_classification_allowed: true`. Otherwise keep `classification_status: "draft classification"` and use `required_next_actions` to complete the remaining coverage, literature review, overlay, semantic validation, or validator work.
...
The default validator mode is strict. A lightweight integration can run `--mode minimal`, but ToolUniverse ACMG final classification workflows should use the default strict mode. The validator also runs `scripts/acmg_semantic_combiner.py`; final labels require both trace validation and `semantic_combiner_status: PASS`.

Validator outcomes:

- `PASS`: final classification may be presented only if the semantic combiner also returns `PASS` and the final-answer guard allows the label.
