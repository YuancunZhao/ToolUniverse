...
try:
    from tooluniverse.acmg_gate.final_label_detector import contains_final_acmg_label
    ...
    contains_final_acmg_label = _module.contains_final_acmg_label

try:
    from tooluniverse.acmg_gate.fixture_utils import load_fixture_manifest, summarize_by_category
except Exception:  # pragma: no cover - direct file execution.
    _fixture_utils_path = _canonical_detector_path()
    if _fixture_utils_path is not None:
        _fixture_utils_path = _fixture_utils_path.with_name("fixture_utils.py")
    if _fixture_utils_path is None or not _fixture_utils_path.exists():
        load_fixture_manifest = lambda _path: {}
        summarize_by_category = lambda _results: {}
    else:
        _fixture_spec = importlib.util.spec_from_file_location("acmg_fixture_utils", _fixture_utils_path)
        if _fixture_spec is None or _fixture_spec.loader is None:
            raise
        _fixture_module = importlib.util.module_from_spec(_fixture_spec)
        _fixture_spec.loader.exec_module(_fixture_module)
        load_fixture_manifest = _fixture_module.load_fixture_manifest
        summarize_by_category = _fixture_module.summarize_by_category
...
def has_final_like_label(text: str) -> bool:
    if FINAL_CONTEXT_RE.search(text):
...
def main(argv: list[str] | None = None) -> int:
    ...
    for path in sorted(fixtures_dir.glob("*.json")):
        ...
        row["category"] = categories.get(path.name, "uncategorized")
        results.append(row)
