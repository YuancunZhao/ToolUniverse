...
7. `semantic_combiner_validation`: recompute whether the reported final label is supported by `current_counted_evidence_resolved`; unsupported labels fail validation.
...
5. **Clinical context intake**
   ...
   - User-provided `family_context`, `phenotype_context`, `disease_context`, `inheritance_context`, zygosity, and phase context may trigger route candidates for de novo, segregation, PM3 in-trans, PP4, BS2, BP2, or BP5. These rows must use `source_type: user_context`, `counted: false`, and `route_outcome: overlay_required` or `candidate_only` until the criterion-specific validator passes.
