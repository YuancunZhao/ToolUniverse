...
| **Drug choice for a described patient** — "which of the following is most appropriate for", "which drug should this patient receive", a vignette naming a modifier (**hepatic/renal impairment**, Child-Pugh, "on a strong CYP3A4 inhibitor", pregnancy, a contraindication) and offering candidate drugs. The deciding fact is in each candidate's FDA label, not recall — resolve every option brand→generic first (two options can be the same drug), then check the label section the question turns on: `FDA_get_active_ingredient_info_by_drug_name`, `FDA_get_indications_by_drug_name`, `FDA_get_pharmacokinetics_by_drug_name`, `FDA_get_drug_interactions_by_drug_name` | `Skill(skill="tooluniverse-biomedical-fact-lookup")` |
